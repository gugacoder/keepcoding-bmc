...
export function AiSuggestionCard({
  ...
  return (
    <div
      className={`relative bg-amber-50/50 dark:bg-amber-950/10 rounded-2xl border border-amber-200/40 dark:border-amber-800/30 shadow-sm transition-all duration-500 ${fading ? 'opacity-0 scale-95' : 'opacity-100 scale-100'}`}
    >
      {/* AI Badge */}
      ...
      <div className="p-3 space-y-2 pr-14">
        ...
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          <Badge color="orange">{TYPE_LABEL[item.type] ?? item.type}</Badge>
          <span>·</span>
