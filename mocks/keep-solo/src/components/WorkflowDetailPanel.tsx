...
export function WorkflowDetailPanel({ workflow, onClose }: Props) {
  ...
  return (
    <div className="flex flex-col h-full bg-white rounded-3xl border border-amber-100 shadow-lg relative overflow-hidden">
      {/* Heartbeat activation overlay */}
      {activating && (
        <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-white/95 backdrop-blur-sm rounded-3xl">
          <div className="relative flex items-center justify-center mb-6">
            ...
            />
            <div
              className="relative w-20 h-20 bg-gradient-to-br from-amber-500 to-orange-500 rounded-full flex items-center justify-center shadow-2xl shadow-amber-300"
          ...
          <p className="text-base font-semibold text-amber-700 animate-pulse">Ativando heartbeat…</p>
          <p className="text-sm text-stone-400 mt-1">Agente assumindo operação autônoma</p>
      ...
      <div className="flex items-start justify-between px-5 pt-5 pb-4 border-b border-amber-100">
        <div>
          <h2 className="text-base font-semibold text-stone-900 leading-tight">{workflow.name}</h2>
          <p className="text-xs text-stone-400 mt-0.5">{workflow.description}</p>
        </div>
        <button
          onClick={onClose}
          className="p-1.5 rounded-xl text-stone-400 hover:text-stone-600 hover:bg-amber-50 transition-colors ml-3 shrink-0"
        >
          <X size={16} />
        </button>
      </div>
      ...
      <div className="px-5 pt-5 pb-4">
        <p className="text-xs font-semibold text-stone-400 uppercase tracking-wide mb-4">Pipeline de deploy</p>

        <div className="relative">
          {/* connector line */}
          <div className="absolute top-4 left-4 right-4 h-0.5 bg-amber-100" />
          <div
                ...
                <div key={step.status} className="flex flex-col items-center gap-1.5" style={{ width: '14.28%' }}>
                  <div
                    className={`
                      ...
                      ${active && isHeartbeatStep ? 'bg-gradient-to-br from-amber-400 to-orange-500 border-orange-500 shadow-sm shadow-amber-200' : ''}
                      ${active && !isHeartbeatStep ? 'bg-white border-amber-500 shadow-sm shadow-amber-100' : ''}
                  ...
                  >
                    {done ? (
                      ...
                      <Icon
                        size={15}
                        weight="duotone"
                        className={active ? 'text-amber-600' : 'text-stone-300'}
                      />
                    )}
                  </div>
                  ...
                  <span
                    className={`text-center leading-tight text-[10px] font-medium
                      ${active && isHeartbeatStep ? 'text-orange-700' : ''}
                      ${active && !isHeartbeatStep ? 'text-amber-700' : ''}
                      ${done ? 'text-stone-600' : ''}
      ...
      <div className="px-5 flex-1 overflow-y-auto">
        ...
        {workflow.status === 'agente_pronto' && !activating && (
          <div className="mt-4 mb-4">
            <div className="mb-4">
              <div className="flex items-center justify-between mb-1.5">
                <span className="text-sm font-medium text-orange-700">Treinamento completo</span>
                <span className="text-xs text-orange-600 font-mono">100%</span>
              </div>
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 px-4 py-3 text-sm font-semibold text-white bg-gradient-to-r from-amber-500 to-orange-500 rounded-2xl hover:from-amber-600 hover:to-orange-600 active:from-amber-700 active:to-orange-700 transition-all shadow-md shadow-amber-200"
            >
        ...
        {heartbeatActive && !activating && (
          <div className="flex flex-col items-center py-8 gap-4">
            <div className="relative flex items-center justify-center">
              ...
              <div className="relative w-14 h-14 bg-gradient-to-br from-amber-500 to-orange-500 rounded-full flex items-center justify-center shadow-lg shadow-amber-200 z-10">
                <Robot size={28} weight="duotone" className="text-white" />
              </div>
            </div>
            <div className="text-center">
              <p className="text-base font-semibold text-orange-700">Agente ativo</p>
              <p className="text-sm text-stone-500 mt-0.5">Heartbeat pulsando — operando de forma autônoma</p>
            </div>
          </div>
        )}
      ...
      {/* Footer meta */}
      <div className="px-5 py-3 border-t border-amber-100 flex items-center gap-2 text-xs text-stone-400">
        <span>
          Criado em{' '}
        ...
        <ArrowRight size={11} />
        <span className="text-amber-500 font-medium">{workflow.name}</span>
      </div>
    </div>
  )
}
...
function ActionButton({ onClick, label }: { onClick: () => void; label: string }) {
  return (
    <button
      onClick={onClick}
      className="w-full flex items-center justify-center gap-2 px-4 py-3 text-sm font-semibold text-white bg-gradient-to-r from-amber-500 to-orange-500 rounded-2xl hover:from-amber-600 hover:to-orange-600 transition-all shadow-sm shadow-amber-200"
    >
      {label}
